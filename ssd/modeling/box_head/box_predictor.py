...
class BoxPredictor(nn.Module):
    ...
    def forward(self, features):
        ...
        for feature, cls_header, reg_header in zip(features, self.cls_headers, self.reg_headers):
            cls_logits.append(cls_header(feature).permute(0, 2, 3, 1).contiguous())# permute更换tensor的shape之后，contiguous()作为一个深复制的办法，避免修改变换shape之前的内容
            bbox_pred.append(reg_header(feature).permute(0, 2, 3, 1).contiguous())

        # # block the output of extra layers
        # if self.select_layer in [0,1,2,3,4,5]:
        #     for i in range(len(cls_logits)):
        #         if i != out:
        #             cls_logits[i] = torch.zeros_like(cls_logits[i])
...
class SSDBoxPredictor(BoxPredictor): # 直接使用CNN回归结果： cls_block和reg_block
    def cls_block(self, level, out_channels, boxes_per_location):
...
def make_box_predictor(cfg):
    return registry.BOX_PREDICTORS[cfg.MODEL.BOX_HEAD.PREDICTOR](cfg) # _C.MODEL.BOX_HEAD.PREDICTOR = 'SSDBoxPredictor' in ssd/defaults.py
