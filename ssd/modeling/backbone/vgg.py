...
def add_extras(cfg, i, size=300):
    ...
    for k, v in enumerate(cfg):
        if in_channels != 'S':
            if v == 'S': # 'S' 表示的是stride，extras_base中在S字母的下一个元素是output channels大小，feature map的大小是会下降的
...
class VGG(nn.Module):
    def __init__(self, cfg):
        ...
        vgg_config = vgg_base[str(size)] # defines the number of output channels for each layer in vgg backbone
        extras_config = extras_base[str(size)] # defines the number of output channels for each layer in extra feature maps
...
def vgg(cfg, pretrained=True):
    ...
    if pretrained:
        ...
        if cfg.MODEL.BACKBONE.ISFREEZE: # 冻结vgg backbone的参数不训练
            model.vgg.weight.requires_grad = False
    return model
