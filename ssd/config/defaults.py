...
_C.MODEL.NEG_POS_RATIO = 3 # negative : positive = 3:1
...
_C.MODEL.BACKBONE.ISFREEZE = False
_C.MODEL.BACKBONE.RFBTYPE = 'NONE' # NONE or Advanced or Basic or Original
# for hrnet
_C.MODEL.BACKBONE.C = 32
...
# ---------------------------------------------------------------------------- #
_C.MODEL.NECK = CN()
_C.MODEL.NECK.NAME = 'NONE'

# -----------------------------------------------------------------------------
# PRIORS
# -----------------------------------------------------------------------------
_C.MODEL.PRIORS = CN()
_C.MODEL.PRIORS.FEATURE_MAPS = [38, 19, 10, 5, 3, 1] # 需要依据网络的特征图大小来进行调节
_C.MODEL.PRIORS.STRIDES = [8, 16, 32, 64, 100, 300]
# MIN_SIZES 和 MAX_SIZES 是由 ratio 等参数计算出来的, 但是 MIN_SIZES 和 MAX 是依据数据集的尺度分布来计算的, 决定不同特征图上prior box的边长
_C.MODEL.PRIORS.MIN_SIZES = [30, 60, 111, 162, 213, 264] # these parameters are for voc0712 dataset 
...
_C.SOLVER.LR = 1e-3 # original 1e-3
_C.SOLVER.MOMENTUM = 0.9
_C.SOLVER.WEIGHT_DECAY = 5e-4
_C.SOLVER.WARMUP_FACTOR = 1.0 / 3 # original 1.0 / 3
_C.SOLVER.WARMUP_ITERS = 500 # original 500
